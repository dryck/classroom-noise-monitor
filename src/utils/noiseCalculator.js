 ...
 */
export function calculateNoiseLevel(frequencyData) {
 ...
 */
export function smoothNoiseLevel(currentLevel, newLevel, smoothingFactor = 0.3) {
    return currentLevel * (1 - smoothingFactor) + newLevel * smoothingFactor;
}
/**
 * Get color based on noise level relative to threshold
 */
export function getNoiseColor(level, threshold) {
 ...
 */
export function getNoiseStatus(level, threshold) {
    if (level > threshold)
        return 'Too Loud!';
